...
plot_reconstruct_fun <- function(svd, 
  ...
  if(transpose == FALSE) {
    if(age_group == "adults"){
      x_names <- capacity_names_ad
      y_names <- character_names_ad
    }
    if(age_group == "children"){
      x_names <- capacity_names_ch
      y_names <- character_names_ch
    }
  } else if(transpose == TRUE) {
    if(age_group == "adults"){
      x_names <- character_names_ad
      y_names <- capacity_names_ad
    }
    if(age_group == "children"){
      x_names <- character_names_ch
      y_names <- capacity_names_ch
    }
  }
...
d_ad %>%
  gather(capacity, mean_rating, -character) %>%
  # begin mean-centering by capacity
  group_by(capacity) %>%
  mutate(cap_mean = mean(mean_rating),
         mean_cent = mean_rating - cap_mean) %>%
  ungroup() %>%
  # end mean-centering
...
# check mean-centering: column means should be 0
# d_ad %>%
#   gather(capacity, mean_rating, -character) %>%
#   # begin mean-centering by capacity
#   group_by(capacity) %>%
...
#          mean_cent = mean_rating - cap_mean) %>%
#   ungroup() %>% 
#   distinct(character, capacity, mean_cent) %>% 
...
d_ad %>%
  gather(capacity, mean_rating, -character) %>%
  # begin mean-centering by character
  group_by(character) %>%
  mutate(cap_mean = mean(mean_rating),
         mean_cent = mean_rating - cap_mean) %>%
  ungroup() %>%
