...
U_fun <- function(svd,
                  age_group = c("adults", "children", 
                                "children_comb_char", "children_comb_cap"),
                  transpose = FALSE,
                  multiply = FALSE){
  ...
  if(transpose == FALSE) {
    ...
    if(age_group == "children_comb_char"){
      y_names <- c(paste(character_names_ch, "79", sep = "_"),
                   paste(character_names_ch, "46", sep = "_"))
    }
    if(age_group == "children_comb_cap"){y_names <- character_names_ch}
  } else if(transpose == TRUE) {
    if(age_group == "adults"){y_names <- capacity_names_ad}
    if(age_group == "children"){y_names <- capacity_names_ch}
    if(age_group == "children_comb_char"){y_names <- capacity_names_ch}
    if(age_group == "children_comb_cap"){
      y_names <- c(paste(capacity_names_ch, "79", sep = "_"),
                   paste(capacity_names_ch, "46", sep = "_"))
    }
  }  
...
plot_U_fun_prop <- function(svd, 
  ...
  if(efa_colors == TRUE){
    ...
    if(age_group == "children_comb_char"){colors <- efa_colors_79}
    if(age_group == "children_comb_cap"){
      colors <- efa_colors_79 %>%
        ungroup() %>%
        mutate(capacity = paste0(capacity, "_79")) %>%
        bind_rows(efa_colors_79 %>%
                    ungroup() %>%
...
reconstruct_fun <- function(svd,
  ...
  if(age_group == "children_comb_cap"){
    capacity_names <- c(paste(capacity_names_ch, "79", sep = "_"),
                        paste(capacity_names_ch, "46", sep = "_"))
    character_names <- character_names_ch
  }
